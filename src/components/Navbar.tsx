...
const Navbar = () => {
  ...
  // Use solid background when scrolled or menu open, else transparent
  const navBgClass = isOpen || isScrolled ? "bg-white shadow-none" : "bg-transparent";
  ...
  return (
    ...
    >
              ...
              if (item.isDropdown) {
                return (
                  <DropdownMenu key={item.name}>
                    ...
                    <DropdownMenuContent className="w-80 bg-white border border-border shadow-none">
      ...
      {/* OVERLAY: behind drawer (z-40) so it dims page but NOT the drawer */}
      <div
        aria-hidden={!isOpen}
      ...
      {/* DRAWER: above overlay (z-50), fully opaque and no shadow */}
      <div
        ref={panelRef}
        role="dialog"
        aria-modal="true"
      ...
      >
        <aside
          className={`pointer-events-auto absolute top-0 right-0 h-full w-full sm:max-w-md bg-white shadow-none transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}
        >
